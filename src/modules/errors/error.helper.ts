...
function prismaErrorHandler(error: Prisma.PrismaClientKnownRequestError): AppError {
	...
	switch (error.code) {
		case 'P2002':
			return new AppError('Запись уже существует', 409, 'UNIQUE_CONSTRAINT', {
				field: meta.target || 'unknown',
			});
		case 'P2003':
			return new AppError('Сбой ограничения внешнего ключа', 400, 'FOREIGN_KEY_CONSTRAINT', {
		...
		case 'P2000':
			return new AppError('Значение слишком длинное для колонки', 400, 'VALUE_TOO_LONG', {
				field: meta.column_name || 'unknown',
			});
		case 'P2011':
			return new AppError('Обязательное поле не может быть null', 400, 'NULL_CONSTRAINT', {
		...
		case 'P2004':
			return new AppError('Нарушение ограничений базы данных', 400, 'CONSTRAINT_VIOLATION', {
				constraint: meta.constraint || 'unknown',
			});
		case 'P2014':
			return new AppError(
		...
		default:
			return new AppError('Ошибка базы данных', 500, 'DATABASE_ERROR', meta);
	}
}

function validationErrorHandler(error: FastifyError): AppError {
	if (!error.validation) {
		return new AppError('Ошибка валидации данных', 400, 'VALIDATION_ERROR');
	}
	...
	return new AppError('Ошибка валидации данных', 400, 'VALIDATION_ERROR', { details: details });
}
...
export function errorHandler(error: unknown) {
	...
	if (error instanceof Prisma.PrismaClientValidationError) {
		return new AppError('Недопустимые данные запроса', 400, 'VALIDATION_ERROR');
	}

	if (error instanceof Prisma.PrismaClientInitializationError) {
		return new AppError('Ошибка инициализации базы данных', 500, 'DATABASE_INIT_ERROR', {
